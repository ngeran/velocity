// =============================================================================
// settings/config/MotionConfig.qml — Motion token singleton
// =============================================================================
//
// Port of ryoku-arch's ui/Singletons/Tokens.qml motion section (trimmed to
// what the settings shell needs). Four durations, two curves, one global scale — nothing
...
//
// SYNC WITH: bar/config/MotionConfig.qml — token values, dur(), and the
// intake mechanism MUST stay identical (separate processes; only the watched
// file name differs: settings-config.json vs bar-config.json).
// =============================================================================
...
Item {
    ...
    readonly property string configPath: (StandardPaths.writableLocation(StandardPaths.HomeLocation).toString() + "/.config/quickshell/settings-config.json").replace("file://", "")
