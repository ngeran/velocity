...
ShellRoot {
    ...
    // Power menu toggle (for Hyprland keybind)
    IpcHandler {
        id: powerMenuIpc
        target: "powerMenuToggle"

        function toggle() {
            // Find the PowerIcon and toggle its menu
            const powerIcon = findChild("powerIcon")
            if (powerIcon) {
                powerIcon.menuVisible = !powerIcon.menuVisible
                console.log("[PowerMenu] Toggled via IPC:", powerIcon.menuVisible)
            }
        }
    }
